...
We have prepared an RNA-seq dataset uses cases that appeared in The Cancer Genome Atlas (TCGA) breast cancer cohort. The purpose of the tutorial is to:-
...
[https://stemangiola.github.io/rpharma2020_tidytranscriptomics/articles/tidytranscriptomics.html](https://stemangiola.github.io/rpharma2020_tidytranscriptomics/articles/tidytranscriptomics.html)
...
brca_tidy %>% 
  ...
  dplyr::select(.feature,counts) %>% slice(1:10)
```
...
It would be perfectly possible to make the plot using standard `ggplot2` code. However, the `EnhancedVolcano` package simplifies the process and offers some additional features such as automatically labelling the significant genes.
