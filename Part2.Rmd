...
We will now concentrate on the analysis of the tumour samples only, but save the whole object for later sections.
...
If we haven't seen so already, the biggest source of variation is whether a tumour is *estrogen receptor* positive or not. 

A useful sanity check is whether the estrogen receptor gene *ESR1* shows a difference in expression between groups. But looking at our data, how do we know which counts correspond to ESR1?

```{r}
brca_t
```

At the moment we don't know what values to use. All we have is an Ensembl ID, and not a more recognisable naming scheme. Fortunately, we have ways to convert between.

```{r}
ens_ids <- pull(brca_t, .feature)
```


```{r eval=FALSE}
## This is expected to give an error
...
```
```{r}
ids <- data.frame(ens_ids = unique(ens_ids))
ids
```
...
```{r}
brca_t <- brca_t %>% 
  tidyr::separate(.feature, into=c("ENSEMBL","V")) %>% 
...
```{r}
brca_t %>% 
  dplyr::select(1:3, last_col(),last_col(1))
```
We can now query the ESR1 gene, and thankfully it shows a huge difference
...
counts_de <- counts_filtered %>% 
  filter(er_status_by_ihc %in% c("Positive","Negative")) %>% 
    test_differential_abundance(
      ...
      .contrasts = c("er_status_by_ihcPositive - er_status_by_ihcNegative")
    )
```

```{r}
counts_de %>% pivot_transcript() %>% View
